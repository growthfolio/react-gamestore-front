...
export function ProdutoComercialModal({
  produto,
  onClose,
  onSave,
  loading,
}: ProdutoComercialModalProps) {
  const [preco, setPreco] = useState<string>(
    produto.preco?.toString() || "59.99"
  );
      ...
      <div className="card-gaming w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <FormInput
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setEstoque(Math.max(0, parseInt(e.target.value) || 0))
            }
            min={0}
            required
          />
          ...
          <div>
            ...
            <div className="flex justify-between text-sm mt-2">
              ...
              <span className="text-warning-500 font-bold font-gaming">
                {desconto}%
              </span>
            ...
            {desconto > 0 && (
              <p className="text-center mt-3 text-accent-400 font-gaming">
                Preço final:{" "}
                <span className="font-bold text-lg price-gaming">
                  {calcularPrecoFinal()}
                </span>
              </p>
            )}
          ...
          {!ativo && estoque > 0 && parseFloat(preco.replace(",", ".")) > 0 && (
            <div className="p-4 bg-warning-500/10 border border-warning-500/30 rounded-gaming">
              <p className="text-warning-500 text-sm font-gaming">
                ⚠️ Este produto tem preço e estoque definidos mas está{" "}
                <strong>INATIVO</strong>. Ative-o para que os clientes possam
                comprá-lo.
              </p>
            </div>
          )}
